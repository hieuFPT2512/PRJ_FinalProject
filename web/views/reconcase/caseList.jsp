...
<div class="card">
    <div class="card-header"><h2>📁 Danh sách Case xử lý</h2></div>
    <div class="card-body">
        <table>
            <thead><tr><th>#</th><th>Đơn #</th><th>Khách hàng</th><th>Loại sai lệch</th><th>Alert #</th><th>Quy tắc</th><th>Phụ trách</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="<%= request.getContextPath() %>/main?action=caseDetail&id=${c.caseId}" class="btn btn-secondary btn-sm">Chi tiết</a>
                        <c:if test="${c.status != 'Closed'}">
                            <a href="<%= request.getContextPath() %>/main?action=caseClose&id=${c.caseId}" class="btn btn-success btn-sm"
                               onclick="return confirm('Đóng case này?')">✔ Đóng</a>
                        </c:if>
                    </td>
                </tr>
                </c:forEach>
                <c:if test="${empty cases}">
                    <tr><td colspan="9" style="text-align:center;color:#aaa;">Không có case nào</td></tr>
                </c:if>
            </tbody>
        </table>
    </div>
